pub fn part_1(input: &str) -> String {
    filter(input, 5).take(8).map(|(a, _)| hex(a)).collect()
}

pub fn part_2(input: &str) -> String {
    let mut found = 0;
    let password = filter(input, 5)
        .filter(|&(a, _)| {
            1u8.checked_shl(a.into()).is_some_and(|mask| {
                let res = found & mask == 0;
                found |= mask;
                res
            })
        })
        .take(8)
        .fold([None; 8], |mut password, (a, b)| {
            let idx: usize = a.into();
            password[idx] = Some(b);
            password
        });
    password.into_iter().map(|c| hex(c.unwrap())).collect()
}

fn filter(input: &str, k: u32) -> impl Iterator<Item = (u8, u8)> {
    ...
    (0..)
        .map(move |i| md5(prefix, i, &mut num_buf, &mut block, &mut m).to_be())
        .filter(move |&d| d.leading_zeros() >= k * 4)
        .map(|d| {
            let ab: u8 = (d >> 4).try_into().unwrap();
            (ab >> 4, ab & 0xf)
        })
}
...
mod test {
    ...
    #[test_case("abc" => "18f47a30")]
    #[test_case(INPUT => "c6697b55")]
    fn part_1(input: &str) -> String {
        super::part_1(input)
    }

    #[test_case("abc" => "05ace8e3")]
    #[test_case(INPUT => "8c35d1ab")]
