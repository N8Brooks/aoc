...
impl State {
    ...
    fn player_turn(&self, damage_1: u32) -> impl Iterator<Item = Self> {
        ...
        let recharge = self.recharge.saturating_sub(1);
        self.hp_1
            .checked_sub(damage_1)
            .map(|hp_1| {
                // Spell effects
                [
                    (53, 4, 0, 0, 0, 0),
                ...
                .map(
                    move |(cost, damage, heal, shield_2, poison_2, recharge_2)| State {
                        ...
                        turn: false,
                        hp_1: hp_1 + heal,
                        hp_2: hp_2.saturating_sub(damage),
                        mana: mana - cost,
                        shield: shield | shield_2,
                        poison: poison | poison_2,
                        recharge: recharge | recharge_2,
                    },
                )
            })
            .into_iter()
    ...
    fn boss_turn(&self, damage_1: u32, damage_2: u32) -> Option<Self> {
        ...
        Some(State {
            spent: self.spent,
            turn: true,
            hp_1: self.hp_1.checked_sub(damage_1 + attack)?,
            hp_2,
